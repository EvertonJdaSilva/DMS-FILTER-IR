...
if ~isempty(Flist_I)
    [Plist,Flist,Llist,alfa,added,index_poll_center,func_eval,Restoration_success] = restoration_phase(Plist,Flist,Llist,alfa,func_F,func_C,grad_C,lbound,ubound,restoration_approach,CacheP,CachenormP,CacheF,cache,func_eval,tol_feasible,tol_match,Pareto_front,iter);
    if Restoration_success
        poll = 0;
        ind_aux_inf = Flist(end,:)>tol_feasible;
        Flist_I     = Flist(end,ind_aux_inf);
        if ~isempty(Flist_I)
            hmax = max(Flist_I(end,:));
        end
    else
        poll = 1;
    end
...
%
         if (sort_filter_par ~= 0)
             if iter == 0 || Restoration_success || poll_step == 0
