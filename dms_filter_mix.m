function [Plist_F,Flist_F,alfa_F,Plist_I,Flist_I,alfa_I,func_eval] = dms_filter_mix(Pareto_front,func_F,file_ini,...
...
%
         if (sort_filter_par ~= 0)
             if iter == 0
                 % [Plist,Flist,alfa] = sort_filter(Plist,Flist,alfa,stop_alfa,tol_stop,spread_option,tol_feasible);
                 [Plist,Flist,alfa] = sort_filter_bk(Plist,Flist,alfa,stop_alfa,tol_stop,tol_feasible,par_ratio);
             else
                 % [Plist,Flist,alfa] = sort_filter(Plist,Flist,alfa,stop_alfa,tol_stop,spread_option,tol_feasible,count_poll_1,count_poll_2,count_poll);
                 [Plist,Flist,alfa] = sort_filter_bk(Plist,Flist,alfa,stop_alfa,tol_stop,tol_feasible,par_ratio,count_poll_1,count_poll);
             end
         end
%        
...
%
            ...
            if oportunistic_approach && pdom == 0 && count_poll_2~=0
                count_d = nd + 1;
            else
                count_d = count_d + 1;
            end
...
%
      if stop_alfa && (sum(logical(alfa < tol_stop))) %(sum(alfa >= tol_stop)==0)
         halt = 1;
      end
...
[~,ind] = find(Flist(end,:)<= tol_feasible);
ind_aux_I = 1:size(Flist,2);
ind_aux_I(ind) = [];
Plist_aux = Plist;
Flist_F = Flist(1:end-1,ind);
Plist_F = Plist(:,ind);
alfa_F  = alfa(ind);
Flist_I = Flist(:,ind_aux_I);
Plist_I = Plist(:,ind_aux_I);
alfa_I  = alfa(ind_aux_I);
%
...
fprintf('| #iter | #isuc | list size | fpoints | #fevals |\n');
print_format = ['| %5d | %5d |   %5d   |   %5d   |  %5d  |\n\n'];
fprintf(print_format, iter, iter_suc, size(Plist_aux,2), length(ind), func_eval);
%
if output
%   
