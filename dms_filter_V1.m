function [Plist_F,Flist_F,alfa_F,func_eval] = dms_filter_V1(Pareto_front,func_F,file_ini,...
...
%
               ...
               if match
                   ...
                   [pdom,index_ndom] = paretodominance(Ftemp,Flist);
                   if (pdom == 0)
                       if sum(index_ndom)>=1 && h>tol_feasible 
                           improving = 1;
                       end
                       code_add = 1;
                       if (Pareto_front == 1)
...
%     Update hmax
%
      if Flist(end,end)>tol_feasible
          hmax = update_hmax(Flist,CacheF,improving,hmax)
          ind_aux_hmax          = find(Flist(end,:)>hmax);
          Flist(:,ind_aux_hmax) = [];
          Plist(:,ind_aux_hmax) = [];
          Llist(ind_aux_hmax)   = [];
          alfa(ind_aux_hmax)    = [];
...
Plist_F = Plist(:,ind);
alfa_F  = alfa(ind);
% pareto dominance - Only remain with nondominated points
Fpoints_list = Flist_F(:,1);
Ppoints_list = Plist_F(:,1);
Apoints_list = alfa_F(1);
for k = 2:size(Flist_F,2)
    Ftemp    = Flist_F(:,k);
    Ptemp    = Plist_F(:,k);
    alfatemp = alfa_F(k);
    [pdom,index_ndom] = paretodominance(Ftemp,Fpoints_list);
    if (pdom == 0)
        Fpoints_list = [Fpoints_list(:,index_ndom),Ftemp];
        Ppoints_list = [Ppoints_list(:,index_ndom),Ptemp];
        Apoints_list = [Apoints_list(index_ndom),alfatemp];
    end
end
Flist_F = Fpoints_list;
Plist_F = Ppoints_list;
alfa_F  = Apoints_list;
