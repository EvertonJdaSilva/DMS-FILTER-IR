...
if F(end,1)>tol_feasible
    ...
    if ~pdom_L
        ...
        indices_L1 = find(indices);
        indices_L2 = find(~indices);
        L1         = Infeasible(:,indices);
        L2         = Infeasible(:,~indices);
        ...
        index_ndom_Infeasible(indices_L1) = index_ndomL1;
        index_ndom_Infeasible(indices_L2) = index_ndomL2;        
        if ~isempty(Feasible)
            index_ndom_Feasible = ones(1,size(Feasible,2));
        else
            index_ndom_Feasible = [];
        end
        index_ndom(inf_feas)   = logical(index_ndom_Feasible);
        index_ndom(inf_infeas) = logical(index_ndom_Infeasible);
    else
        index_ndom = [];
    end    
else
    ...
    if (sum(index) < mlist)
        ...
        if ~isempty(Infeasible)
            index_ndom_Infeasible = ones(1,size(Infeasible,2));
        else
            index_ndom_Infeasible = [];
        end
        index_ndom(inf_feas)   = logical(index_ndom_Feasible);
        index_ndom(inf_infeas) = logical(index_ndom_Infeasible);
    end    
