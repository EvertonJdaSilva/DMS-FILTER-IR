...
nprobs=214;
for app=1:5
    name1 = ['Results\dms_filter_IR1_ap' num2str(app) '_500funceval_V4.m']; % instrucao Windows
    % name1 = ['Results/dms_filter_IR1_ap' num2str(app) '_20000funceval.m']; % instrucao Linux
    name2 = ['dms_filter_IR1_ap' num2str(app) '_500funceval_V4_%s_%s=['];
    fp = fopen(name1,'wt');     
    func_evals=zeros(nprobs,1);
    for i=1:nprobs
        fprintf('==================  Problema %3d ================== \n' ,i)
        fprintf(fp,name2,problem(i).name,problem(i).Constraint);
        file_cache = ['dms_filter_IR1_ap' num2str(app) '_500funceval_V4_' problem(i).name '_' problem(i).Constraint '_cache.m'];   
        lb     = problem(i).lbound;
        ub     = problem(i).ubound;   
        xi     = problem(i).x_ini;
        ...
        [Plist_F,f,alfa_F,func_eval] = dms_filter_IR1(1,func_F,[],file_cache,xi,lb,ub,func_C,grad_C,app);
