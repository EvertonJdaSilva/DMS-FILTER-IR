function [pdom,index_ndom] = paretodominance_new(F,Flist,tol_feasible)
%
% Purpose:
%
...
if Flist(end,1)>tol_feasible && F(end)<Flist(end,1)
    ...
    % If the poll center is not dominated verifies dominance with the all 
    % points in the list
    if ~poll_center_dom
        index_ndom    = [];
...
elseif Flist(end,1)<=tol_feasible || F(end)>=Flist(end,1)
    index_ndom    = [];
    [nlist,mlist] = size(Flist);
    Faux          = repmat(F,1,mlist);
    index1        = logical(sum(logical(Faux<Flist)));
